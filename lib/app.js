...
export const appManager = {
  ...
  // ! USE WITH CAUTION ! //
  removeAllCourses: async () => {
    if (
      confirm(`Do you want to permanently remove all courses from the system?`)
    ) {
      alert(`You have permanently removed all courses from the system.`);
      const getData = await httpClient.get('courses');

      getData.forEach(async (data) => {
        await httpClient.remove(`courses/${data.id}`);
      });
    }
  },
  // ! ================ ! //
