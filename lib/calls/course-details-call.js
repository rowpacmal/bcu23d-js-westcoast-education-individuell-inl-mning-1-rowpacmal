...
export const courseDetailsCall = () => {
  ...
  const enrollCourse = async () => {
    ...
    if (key === 'admin') {
      const statusText = `The admin can't enroll in courses.`;
      ...
      if (enrollment) {
        const statusText = 'You have already enrolled in this course.';
        sendStatusMassage(statusText, 'warning');
      } else {
        ...
        const statusText = `Congratulation, you have enrolled in "${course.title}"!`;
